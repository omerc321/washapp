...
-   **QR Code Payment Feature**: Implemented complete QR code payment system enabling cleaners to generate scannable payment links for instant job assignment. Cleaners can generate unique payment tokens with QR codes via their dashboard. Customers scan the QR code to access a pre-filled payment page at `/customer/pay/:token` showing cleaner and company details. Upon successful payment, the job automatically assigns to the cleaner who generated the token. Tokens are single-use, expire after 1 hour, and are validated server-side. Database: Added `cleanerPaymentTokens` table with token lifecycle management (create, validate, mark as used, cleanup expired). Backend: POST `/api/cleaner/generate-qr-token`, GET `/api/customer/payment-token/:token`, modified `/api/confirm-payment/:id` for auto-assignment logic. Frontend: QR generator modal with `react-qr-code`, copy link functionality, and countdown timer in cleaner dashboard.
