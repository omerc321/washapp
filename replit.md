...
-   **Authentication**: Passport.js with bcrypt for email/password (staff roles), `connect-pg-simple` for PostgreSQL session storage. Auto-admin setup on server startup ensures production admin account exists with correct credentials. Session cookies configured with `sameSite: 'none'` in production for cross-origin requests, `trust proxy: 1` enabled for HTTPS load balancers. Password reset functionality with secure token-based flow, 1-hour expiration, and email notifications via Resend. Customer data model uses email as primary identifier (required, unique), with optional phone number field (non-unique) to allow multiple customers sharing the same phone.
...
-   **Push Notifications**: Web Push API for real-time alerts to customers and cleaners. Cleaners receive notifications based on four-way geolocation validation: (1) job location within service area, (2) cleaner assigned to that service area, (3) cleaner ON_DUTY status, (4) cleaner's current GPS location also within the same service area boundaries. Location staleness checked (10-minute window). Features include: sound preferences, stale subscription cleanup (410/404 responses), structured error logging with delivery summaries, parallel geofence eligibility checks to avoid N+1 queries, and post-subscribe UI synchronization.
...
-   **Idempotency Pattern**: Critical mutation endpoints (job completion) use atomic conditional updates (`completeJobIfNotCompleted`) in storage layer with `WHERE status != COMPLETED` clauses to prevent duplicate receipts/emails from rapid double-submissions. Only the first completion request succeeds; subsequent attempts return early with success status but skip side effects.
...
-   **Cleaner Flow**: Invitation-based registration, streamlined shift-based status (starting shift automatically sets ON_DUTY, ending shift sets OFF_DUTY), job acceptance, "Open in Google Maps" navigation to job location, and photo-based job completion. Compact header design with 40% reduced height. Continuous GPS location tracking for on-duty cleaners (currentLatitude, currentLongitude, lastLocationUpdate). Job notifications require cleaner's current location to be within the same geofence as the job location. Password reset available via "Forgot password?" link on login page. Automatic shift timeout after 10 minutes of no location updates (prevents ghost shifts when session ends unexpectedly).
...
-   **Job Acceptance**: Stripe payment triggers PENDING_PAYMENT, webhook confirmation to PAID, job becomes available to on-duty cleaners who meet ALL four criteria: (1) job location is within company's service area, (2) cleaner is assigned to that specific service area, (3) cleaner is ON_DUTY, (4) cleaner's current GPS location is within the same service area boundaries. Manual acceptance is first-come-first-served with WebSocket updates.
...
-   **Auto-Shift-Timeout**: Cleaners' shifts automatically end if no location update is received for 10+ minutes (prevents "ghost shifts" when cleaner's session ends unexpectedly). Service runs every minute checking for stale ON_DUTY cleaners, calculates shift duration, and sets status to OFF_DUTY. Implemented in `server/autoShiftTimeoutService.ts`.
