...
-   **QR Code Payment Feature**: Implemented streamlined QR code payment system for instant on-site bookings. Cleaners generate unique payment tokens with QR codes via their dashboard. Customers scan the QR code to access `/customer/pay/:token` where they see company/cleaner details and a simple inline form to enter car details (plate number, parking number, phone). Upon form submission, the page redirects directly to checkout showing full payment summary with VAT breakdown. After successful payment, the job auto-assigns to the cleaner who generated the token. Tokens are single-use, expire after 1 hour, and are validated server-side. Database: `cleanerPaymentTokens` table. Backend: POST `/api/cleaner/generate-qr-token`, GET `/api/customer/payment-token/:token`, POST `/api/customer/by-phone` (creates/gets customer by phone only), modified `/api/confirm-payment/:id` for auto-assignment. Frontend: QR generator modal with `react-qr-code`, inline car details form on payment page, direct checkout integration.
